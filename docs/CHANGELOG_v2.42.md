...
## 사후 수정 #3 (2026-07-09) — 🟡 bash jq 엄격 slurp가 깨진 줄 1개에 전체 실패
크로스 리뷰 지적(수정 대기 항목) 반영. Mac/Linux 한정, 오계수 아님(최악 일시 undercount → seen 미갱신이라 다음 Stop서 만회).
- **문제**: `stop-check.sh` retrieval 파싱 `tail -n 60 | jq -rs '[ .[] | ... ]'`는 window에 **깨진 JSON 한 줄만 있어도 전체 parse 실패**(transcript 동시 append 중 tail이 미완성 줄 잡는 케이스). PS는 per-line try/catch로 견뎌 PS/bash 비대칭.
- **수정**: 라인 관용 slurp `jq -rRn '[inputs | fromjson? // empty] | ...'` — 깨진 줄만 skip. live `stop-check.sh` + `pawpad-setup.ps1` 임베디드 템플릿 동기. (statusline.sh 구버전 폴백 경로의 동일 `-rs` 패턴은 숫자 가드로 0 폴백되는 우아한 저하라 미수정.)
- **검증**: jq 1.7.1(win64)로 fixture E2E — 구식 `-rs` 전체실패 재현 → 신식 깨진 줄 skip·`cmap:hit` 기록·seen=uuid·dedupe 무중복·전량 깨진 window graceful(exit 0, stats 불변). `bash -n` OK.
