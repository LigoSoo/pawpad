...
- root: route만, 심볼표 금지.
- keywords: 사용자 표현 -> feature 경로만 (파일/심볼 금지 → 동의어 stale에도 source 무영향). 4KB 초과 시에만 도메인별 분할.
- feature leaf: 실제 수정후보 파일+심볼. leaf 4KB 근접 시 features/{id}/ 하위 ui,data,domain,test로 split.

### Lookup 알고리즘 (최대 3 read)
1. 한국어/자연어 → keywords.md 1개 → feature leaf 1개 → source. (root는 첫 진입만)
2. 영문 심볼 알면 → rg로 .claude/codemap/features 직접 grep (페이지 통째읽기 불필요).
3. 금지: codemap 전체 read / 심볼 줄범위 아는데 source 파일 통째 read.
- 핵심: 분할은 grep lookup 성능 불변(rg는 폴더째 검색) + 실수로 통째읽기만 차단. 다운사이드 없음.
