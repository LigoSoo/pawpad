...
이 문서는 PawPad — Agentic Engineering Toolkit(v2.28 FROZEN)를 설치한 프로젝트에서, **하나의 기능/프로젝트를 기획부터 완성·배포까지** 진행하는 전체 절차와 각 단계의 스킬 사용 예시를 설명한다.
...
```
■ 기획 → 구현 (가장 흔한 흐름)
  /clarity → /grill-me → /to-prd → /mockup → (구현세션) /memory 인수 → 코딩 → DoD → done

■ UI 화면 작업
  /clarity → /design → /to-prd → /mockup(lo→hi) → 구현(/lean-code /codemap) → DoD
...
설치 검증:
- `.claude/skills/*/SKILL.md` 17개 — no-BOM, `---` frontmatter로 시작해야 등록됨
- `.codex/config.json` skills 배열 17개
- `pawpad-setup.ps1` STATUS: FROZEN (v2.28 Unified Claude + Codex Distribution)
...
.agents/skills/{17}/       # Codex repo skill mirror (DO NOT EDIT)
docs/HOOK_TESTING.md       # hook 회귀 체크리스트
pawpad-setup.ps1  # 통합 단일 설치 스크립트 (FROZEN v2.28)
```
