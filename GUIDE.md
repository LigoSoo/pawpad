...
이 문서는 PawPad — Agentic Engineering Toolkit(v2.41 FROZEN)를 설치한 프로젝트에서, **하나의 기능/프로젝트를 기획부터 완성·배포까지** 진행하는 전체 절차와 각 단계의 스킬 사용 예시를 설명한다.
...
| 스킬 | 언제 |
|------|------|
| `/resume` | **세션 시작마다**. `_wip.md`/lane/handoff/meta/codemap 읽고 작업 재개 |
...
```
■ 기획 → 구현 (가장 흔한 흐름)
  /clarity → /grill-me → /to-prd → /mockup → (통합 4탭 검토: /mockup viewer → /viewer-apply 로 스팩 동기) → (구현세션) /resume 인수 → /code-delegate(코딩 위임, 선택) → 코딩 → /security-check → /review(고위험) → DoD → done

■ UI 화면 작업
  /clarity → /design → /to-prd → /mockup(lo→hi) → /code-delegate(코딩 위임, 선택) → 구현(/lean-code /codemap) → /security-check → /review(고위험) → DoD

■ 순수 코드 작업 (사양 명확)
  /resume → /lean-code → /codemap → /caveman → /security-check → /review(고위험) → DoD
...
설치 검증:
- `.claude/skills/*/SKILL.md` 19개 — no-BOM, `---` frontmatter로 시작해야 등록됨
- `.codex/config.json` skills 배열 19개
- `pawpad-setup.ps1` STATUS: FROZEN (v2.41 Unified Claude + Codex Distribution)
...
pawpad-setup.ps1  # 통합 단일 설치 스크립트 (FROZEN v2.41)
```
