...
이 문서는 PawPad — Agentic Engineering Toolkit(v2.36 FROZEN)를 설치한 프로젝트에서, **하나의 기능/프로젝트를 기획부터 완성·배포까지** 진행하는 전체 절차와 각 단계의 스킬 사용 예시를 설명한다.
...
```
■ 기획 → 구현 (가장 흔한 흐름)
  /clarity → /grill-me → /to-prd → /mockup → (구현세션) /resume 인수 → /code-delegate(코딩 위임, 선택) → 코딩 → /security-check → /review(고위험) → DoD → done
...
- `pawpad-setup.ps1` STATUS: FROZEN (v2.36 Unified Claude + Codex Distribution)
...
.agents/skills/{20}/       # Codex repo skill mirror (DO NOT EDIT)
docs/HOOK_TESTING.md       # hook 회귀 체크리스트
pawpad-setup.ps1  # 통합 단일 설치 스크립트 (FROZEN v2.36)
```
