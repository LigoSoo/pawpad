...
이 문서는 PawPad — Agentic Engineering Toolkit(v2.43 FROZEN)를 설치한 프로젝트에서, **하나의 기능/프로젝트를 기획부터 완성·배포까지** 진행하는 전체 절차와 각 단계의 스킬 사용 예시를 설명한다.
...
> v2.31: **문서/lane 토큰 sharding** (작업 단위 커서 2 lane 분리, 의도 동일 → 단일 버전). ① **PRD Area-Sharding**: 프로젝트 PRD를 도메인 영역별 `src/prd/{area}.md`로 분할, `PRD-tree.md`=인덱스(영역 상태 마커 ✓/🔨/⬜), feature-id 라우팅으로 현재 영역만 read. ② **Completed Task Log**: lane ✅완료 작업항목을 `verifications/{feature-id}-tasklog.md`로 이월(미완/진행 전수 + 최근 세션 완료분 + 포인터), 트리거 checkpoint+task-done. 둘 다 v2.30(검증근거)과 동일 "완료/audit 분리·활성만 hot" 원칙, 무손실·instruction 기반.
> v2.32: **clarity 접근법 게이트** — brainstorming "2-3 대안 제시" 메커니즘 이식. clarity 모호도 PASS 직후 접근법 게이트 신설: 실질 대안 ≥2면 2-3 접근법(트레이드오프 + **추천 1개 필수**) 제시 → AskUserQuestion 선택 → 그 경로로 구현계획. 자명 단일이면 게이트 생략(가짜 대안 금지, lean). clarity SKILL.md live+임베드+.agents 미러 동기.
> v2.33: **code-delegate 스킬 신규**(18→19) — 코딩 단계 진입 시 사용자 선택 모델의 코딩 서브에이전트로 위임(설계=상위 Opus, 코딩=하위 모델). 서브 독립 context로 부모 컨텍스트 린 유지 + 토큰 절감. spec/lane 포인터 전달·요약 반환·부모 피드백 루프. CLAUDE/AGENTS 자동제안(구현 진입 경계) 1줄. Claude Code 주력(Task model 오버라이드)·Codex 수동 세션 폴백.
> v2.34: **skill rename `memory` → `resume`** — "기억 저장" 오해를 없애고 세션 시작 재개(ON START) **동작에 직결**되는 이름으로 교정. `/memory` 하드 제거(`/resume`만), 기존 설치는 `-Upgrade` 시 구 `memory/` 디렉토리(.claude+.agents) 자동 마이그레이션 + config skills 배열 치환(ADR-002 선례). 스킬 수 19 불변(rename).
> v2.35: **resume 최소로드** — ON START/세션재개 읽기 토큰 절감. ① HYBRID 조건부화(resume 스킬이 무조건 읽던 협업 프로토콜을 active-lane 있을 때만, CLAUDE/AGENTS와 일치 — 솔로/분석 세션 ~5.8k 절감). ② _meta RECENT skip(_meta를 헤더→BLOCKED→NEXT→RECENT 순 재정렬, ON START는 상단만 부분읽기·완료이력 RECENT는 하단·history 시 on-demand, ~5.5k 절감). 기능·스킬 수 19 불변.
> v2.37: **grill-with-docs → grill-me 흡수**(스킬 20→19) — 죽은 중복 제거. grill-with-docs 고유가치 중 순수 인터뷰 품질만 grill-me에 흡수: 모호 용어 → canonical 좁힘, 엣지케이스 시나리오 stress-test, 진술 vs 코드 모순 표면화, 탐색 시 기존 문서 확인. 미사용 `CONTEXT.md` 용어집·Decision Placement Matrix와 중복인 ADR offering은 DROP. 기존 설치는 `-Upgrade` 시 구 `grill-with-docs/` 디렉토리(.claude+.agents) 자동 제거 + config skills 배열 정리(ADR-002 선례).
> v2.36: **통합 기획 뷰어 (데이터-구동, no-backend)** — 4탭(PRD/기능명세서/메뉴구성도/와이어프레임) 통합 뷰어. 범용 `spec-viewer.html`(데이터 비종속, File System Access API)가 고정명 외부 JSON(`src/viewer/{prd,fts,userflow,wire}.json`)을 **폴더 1회 선택 후 자동 로드 → 편집 → 같은 파일 제자리 저장(다운로드·백엔드 없음) → 재로드**. 선택 폴더 핸들을 IndexedDB에 기억 → 다음에 `↻ 다시 열기` 1클릭 복원(임의 절대경로 자동열기는 브라우저 보안상 불가). PRD/명세가 바뀌면 agent는 **JSON만 수정**(HTML 영구 불변 → 토큰 절감). 승인 액션 없음(JSON에 남은 항목=설계/개발 대상, 삭제=제외), 상태(예정/진행중/완료)는 agent가 구현하며 갱신·읽기전용. `src/viewer/*.json`은 ON START/resume 자동 로드 금지(초기 기획 강화·context 절감). 신규 스킬 `viewer-apply`(19→20, JSON→스팩 동기) + `mockup` viewer 모드. 렌더: 기능명세서=CSS 트리(그룹──트렁크┬─leaf), **메뉴구성도=계층 트리+드래그앤드랍(메뉴 순서·뎁스 재배치, 뎁스별 색)**, **와이어프레임=디바이스 프레임(mobile/web)+컴포넌트 lo-fi(검토·조정 전용)**. 스킬 수 20.
> v2.38: **codemap ON START 부분읽기** — 코드세션 ON START에서 codemap을 MAP+HOT(조망)만 읽고, 전체 심볼표(INDEX)는 특정 심볼 필요 시 Grep으로 그 줄만 가져온다(전체 로드보다 쌈). 더불어 HOT를 spec(최근 3~5개·1줄)대로 정리(28→5, HOT-only 심볼은 INDEX로 강등·무손실). 코드세션 ON START codemap read ~7k→~0.5k tok 절감(절감은 ON START read 부분, INDEX는 grep-on-demand라 미로드) — ON START 절감 3종(_meta·HYBRID·codemap) 완성. 동작·스킬 수(19) 불변.
> v2.39: **번들 선택 설치 + 안내 언어 i18n** — 설치 시 스킬을 기능 번들 단위로 선택(`-Preset lean|standard|full` / `-Bundles prd,ui,delegate,review`, 미지정 시 대화형, Enter=full). Core 11(인프라)는 항상 설치, Optional 4 번들(prd 기획 / ui 설계·목업 / delegate 코딩위임 / review 교차리뷰) 선택 — ui·delegate는 prd 자동 포함(하드의존). 미선택 번들은 스킬 디렉토리 + `config.json`/`SKILLS_MANIFEST` + CLAUDE/AGENTS/HYBRID 본문 참조까지 정합 정리(dangling 0). 설치 안내 메시지 언어 선택(`-Lang en|ko`, Enter=ko). **스킬 내용·CLAUDE.md 등 에이전트 문서는 단일 소스 무변경**, 스킬 수(19) 불변.
> v2.40: **codemap trim-router (small-page)** — codemap 성장전략 flat→trim-router: 대규모(50KB+) `_index.md`를 `_root.md`(route)+`keywords.md`(한국어→feature)+`features/{id}.md`(source pointer)로 분할, cap root 2KB·그외 4KB, generated(`*.g.dart`/`*.freezed.dart`/`lib/generated`) 제외, lookup 알고리즘(최대 3 read)+1줄규율. domain 중간층 제외(leaf 중복·3중쓰기 drift 회피). 통째읽기 사고 ~14k→~1k 봉쇄·grep 성능 불변. account-link pilot 검증. 스킬 수(19) 불변. **v2.40 내 보강(2026-07-01~02)**: ① analyze hook 2단계 fix — `-File` 스크립트(`analyze.ps1`/`analyze.sh`) 통일(Git Bash 디스패치 충돌 제거) + 진단 stderr 재전송·exit 2/0 정규화(PostToolUse는 exit 2의 stderr만 agent 전달 — 설계 이래 미동작이던 Edit 직후 진단 피드백 활성화, 기존 설치 `-Upgrade` 필요). Unix `analyze.sh` 파이프 exit 소실은 `set -o pipefail`+그룹 리다이렉트로 보정. ② codemap 자연어 lookup은 keywords.md **통째 read 후 의미매칭**(grep 정확매칭 금지 — lookup miss→전체 소스 스캔 토큰 폭증 방지). 상세: docs/CHANGELOG_v2.40.md.
> v2.41: **retrieval-source 표시 (A 선언식 + B 계측식)** — agent가 코드를 codemap/ctxdb로 찾았는지, 소스 전체를 뒤졌는지 가시화. ① 선언식: CLAUDE/AGENTS `Response Style`에 `### Retrieval 표시` — 탐색 수행 응답 상단에 `📡 Retrieval: codemap {hit|miss|미사용} | ctxdb {…} | src {read N|full-scan N (사유)}` 1줄(소스 탐색 전 codemap lookup 의무 + full-scan 사유 선언 = codemap-first 행동 유도). ② 계측식(Claude Code 전용): 신규 `read-track` 훅(`PostToolUse: Read|Grep|Glob`)이 read 경로를 cmap/ctx/src로 분류해 `.ctxdb/.state/claude-read-stats`에 누적(toolkit 내부 read 미집계, SessionStart reset), statusline이 `📡 cmap N ctx N src N` 실측 표시 — 전체 소스 스캔(src 폭증)이 자기보고와 무관하게 즉시 관측됨. settings.json `PostToolUse`는 read-track(항상)+analyze(스택 조건부) 2 entry. 기존 설치 `-Upgrade` 반영, 스킬 수(19) 불변. 상세: docs/CHANGELOG_v2.41.md.
> v2.42: **retrieval routing 가시화** — v2.41 계측을 "코드맵을 거쳐 소스에 갔나, 전체를 뒤졌나"가 한눈에 보이게 재설계. statusline `📡 codemap N% · routed X / full-scan Y · src N`: **codemap N%** = 경유율(선언 기반 주지표, 초록≥70/노랑≥40/빨강<40), **src N** = 코드맵 미경유 직접 read 볼륨(위조불가 백스톱, 선언 0+src>0=풀스캔 의심 노랑), **ctx N%** = ctxdb 매칭율(샘플 시). hit/miss는 Stop 훅(stop-check)이 완료 응답의 `📡 Retrieval:` 선언을 파싱해 `.ctxdb/.state/claude-retrieval-stats` 누적(uuid dedupe·`미사용` 분모 제외·형식예시 `{}` 라인 제외·`|` 고정순서 위치분해). 원시 `cmap/ctx` 카운터·route%는 폐기(기계 카운터로는 경유/풀스캔 구분 불가). statusline=UI 렌더=모델 토큰 0. 기존 설치 `-Upgrade`, 스킬 수(19) 불변. 상세: docs/CHANGELOG_v2.42.md.
> v2.43: **task-done 종결 게이트 3종(A+B+C)** — ON TASK DONE 미실행 → stale lane → 다음 세션 resume이 완료 작업을 다음 작업으로 재제안하는 사고(실관측) 계층 방어. **A** 신규 `task-done` 스킬(19→20, Core): 종결 체크리스트(lane→done 이관+_wip 제거+_meta RECENT+tasklog 이월+codemap+commit) 전항 강제, "작업/이슈 종료" 자연어 발동. **B** Stop hook lane-close 백스톱: 응답이 완료를 선언했는데 Active Lanes 잔존 시 decision:block 1회 리마인더(uuid dedupe, 'task-done' 언급 제거 후 매칭으로 오탐 감쇄). **C** resume Lane 신뢰성 게이트: ON START에서 3단 신호(_meta RECENT DONE+lane 잔존=확정 누락 / next steps 전항 체크=완료 의심 / stale updated+실코드 spot-check=사용자 확인)로 대조, 다음 작업 제안 전 실상태 1회 대조 규율. 기존 설치 `-Upgrade`. 상세: docs/CHANGELOG_v2.43.md.
...
└──────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘
   /resume       /clarity       /design        /lean-code     DoD 8개        STATUS
                 /grill-me      /grill-me      /codemap       /task-done     FROZEN
...
```
■ 기획 → 구현 (가장 흔한 흐름)
  /clarity → /grill-me → /to-prd → /mockup → (통합 4탭 검토: /mockup viewer → /viewer-apply 로 스팩 동기) → (구현세션) /resume 인수 → /code-delegate(코딩 위임, 선택) → 코딩 → /security-check → /review(고위험) → DoD → /task-done(종결)

■ UI 화면 작업
  /clarity → /design → /to-prd → /mockup(lo→hi) → /code-delegate(코딩 위임, 선택) → 구현(/lean-code /codemap) → /security-check → /review(고위험) → DoD
...
설치 검증:
- `.claude/skills/*/SKILL.md` 20개 — no-BOM, `---` frontmatter로 시작해야 등록됨
- `.codex/config.json` skills 배열 20개
- `pawpad-setup.ps1` STATUS: FROZEN (v2.43 Unified Claude + Codex Distribution)
...
.agents/skills/{19}/       # Codex repo skill mirror (DO NOT EDIT)
docs/HOOK_TESTING.md       # hook 회귀 체크리스트
pawpad-setup.ps1  # 통합 단일 설치 스크립트 (FROZEN v2.43)
```
