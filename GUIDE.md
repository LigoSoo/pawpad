...
이 문서는 PawPad — Agentic Engineering Toolkit(v2.22 FROZEN)를 설치한 프로젝트에서, **하나의 기능/프로젝트를 기획부터 완성·배포까지** 진행하는 전체 절차와 각 단계의 스킬 사용 예시를 설명한다.
...
│ 세션 재개 │   │ 모호도   │   │ UI/심문  │   │ 코드+상태 │   │ DoD+동결  │   │ freeze   │
└──────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘
   /memory       /clarity       /design        /karpathy      DoD 8개        STATUS
...
| `/karpathy` | 코드 작성 시 **과설계/범위 이탈 방지** 가드레일 |
...
```
/karpathy   ← 요청된 것만. 추가 추상화·범위 밖 파일 수정 금지 확인
...
```
■ 기획 → 구현 (가장 흔한 흐름)
  /clarity → /grill-me → /to-prd → (구현세션) /memory 인수 → 코딩 → DoD → done

■ UI 화면 작업
  /clarity → /design → /to-prd → 구현(/karpathy /codemap) → DoD

■ 순수 코드 작업 (사양 명확)
  /memory → /karpathy → /codemap → /caveman → DoD
...
State enum: `WIP` · `SPEC_READY` · `HANDOFF_TO_CODEX` · `HANDOFF_TO_CLAUDE` · `HANDOFF_TO_NEXT_AGENT` · `BLOCKED`
...
설치 검증:
- `.claude/skills/*/SKILL.md` 15개 — no-BOM, `---` frontmatter로 시작해야 등록됨
- `.codex/config.json` skills 배열 15개
- `pawpad-setup.ps1` STATUS: FROZEN (v2.22 Unified Claude + Codex Distribution)
...
.agents/skills/{15}/       # Codex repo skill mirror (DO NOT EDIT)
