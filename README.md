...
# PawPad — Agentic Engineering Toolkit (v2.39 FROZEN)
...
pawpad-setup.ps1     # 통합 단일 설치 스크립트 (FROZEN v2.39, 번들 선택 -Preset/-Bundles + 안내 언어 -Lang, 설치 UI: paw 배너+live 진행 바+체크리스트, -ShowLog로 상세 로그)
.codex/
...
# 스크립트 STATUS
Get-Content .\pawpad-setup.ps1 -TotalCount 2   # v2.39 Unified ... FROZEN
```
...
- 변경 이력: `docs/CHANGELOG_v2.39.md` (이전: v2.18~v2.38)
- hook 회귀 체크리스트: `docs/HOOK_TESTING.md`
- 협업 프로토콜 상세: `.claude/HYBRID.md`

## 버전 / 변경 정책

`v2.39 FROZEN` (Unified Claude + Codex Distribution). 변경 시 **새 버전 번호 + 변경 보고서 + Codex 리뷰** 절차를 따른다. 동결본을 직접 고치지 않는다.
