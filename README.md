...
# PawPad — Agentic Engineering Toolkit (v2.36 FROZEN)
...
pawpad-setup.ps1     # 통합 단일 설치 스크립트 (FROZEN v2.36, 설치 UI: paw 배너+live 진행 바+체크리스트, -ShowLog로 상세 로그)
.codex/
...
.agents/skills/{20}/SKILL.md     # Codex repo skill mirror (.claude/skills 단일소스, DO NOT EDIT 헤더)
...
├── skills/{20}/SKILL.md         # 스킬 정의 (no-BOM, --- frontmatter)
...
(Get-ChildItem .claude\skills -Directory).Count   # 20
...
# 스크립트 STATUS
Get-Content .\pawpad-setup.ps1 -TotalCount 2   # v2.36 Unified ... FROZEN
```
...
- 변경 이력: `docs/CHANGELOG_v2.36.md` (이전: v2.18~v2.35)
- hook 회귀 체크리스트: `docs/HOOK_TESTING.md`
- 협업 프로토콜 상세: `.claude/HYBRID.md`

## 버전 / 변경 정책

`v2.36 FROZEN` (Unified Claude + Codex Distribution). 변경 시 **새 버전 번호 + 변경 보고서 + Codex 리뷰** 절차를 따른다. 동결본을 직접 고치지 않는다.
