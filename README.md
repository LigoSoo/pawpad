...
### 이 repo를 git clone 한 경우

런타임/작업 데이터(`.ctxdb` 라우터·L1/L2, `codemap/_index.md`, pawpad lane·meta·decisions, handoff/spec 인스턴스)는 **개인 메모리라 배포본에 포함하지 않는다**(`.gitignore`). clone 직후엔 정적 골격(README/TEMPLATE/VERSION)만 있으므로, **루트에서 setup을 한 번 실행해 빈 스켈레톤을 생성**해야 hook/skill이 정상 동작한다:

```powershell
git clone https://github.com/LigoSoo/pawpad.git
cd pawpad
# 누락된 런타임 스켈레톤(INDEX/L1/L2/codemap/_wip/_meta 등) 생성. 기존 골격 파일은 건너뜀
.\pawpad-setup.ps1 -Stack generic
```

> 이미 작업 중인 설치본을 최신 툴킷으로 올릴 때만 `-Upgrade`(사용자 데이터 보존). 신규 clone은 위처럼 일반 실행.
