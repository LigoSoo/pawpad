...
# PawPad — Agentic Engineering Toolkit (v2.28 FROZEN)

Claude Code ⇄ Codex 하이브리드 협업을 위한 **멀티에이전트 상태관리 + 토큰절약 하니스**.
하나의 PowerShell 스크립트로 어떤 프로젝트에든 17개 스킬 · PawPad 상태머신 · 양 런타임 자동화 hook · 키워드 컨텍스트 DB · 7축 코드베이스 맵 · 보안 검증 게이트를 설치한다.
...
#    /memory   → 기존 작업 재개      |   /clarity → 새 기능 기획 시작
```
...
pawpad-setup.ps1     # 통합 단일 설치 스크립트 (FROZEN v2.28, 설치 UI: paw 배너+live 진행 바+체크리스트, -ShowLog로 상세 로그)
.codex/
├── config.json                  # Codex 보조 설정 (skills 17 + context/backup keys)
...
.agents/skills/{17}/SKILL.md     # Codex repo skill mirror (.claude/skills 단일소스, DO NOT EDIT 헤더)
...
├── SKILLS_MANIFEST.md           # 스킬 카탈로그 (17)
├── skills/{17}/SKILL.md         # 스킬 정의 (no-BOM, --- frontmatter)
...
```
아이디어
  └ /clarity        모호도 5차원 스코어 → 임계값까지 재질문 → 구현계획
  ...
  └ /mockup         PRD-tree → 단일 HTML 목업 (lo-fi 와이어프레임 → hi-fi 디자인)
        ...
        ↓ (구현 세션이 lane 인수)
구현
  └ /memory         SPEC_READY lane 인수 (state=WIP)
...
- **교차 검증**: 중요한 변경은 Codex(또는 Claude) **독립 리뷰** → `.claude/pawpad/reviews/`에 PASS/findings 기록 후 반영.
...
```powershell
# 스킬 17개 frontmatter (no-BOM, --- 로 시작)
(Get-ChildItem .claude\skills -Directory).Count   # 17
...
# Codex skills 배열 17
(Get-Content .codex\config.json -Raw | ConvertFrom-Json).skills.Count   # 17

# 스크립트 STATUS
Get-Content .\pawpad-setup.ps1 -TotalCount 2   # v2.28 Unified ... FROZEN
```
...
- 변경 이력: `docs/CHANGELOG_v2.28.md` (이전: v2.18~v2.27)
- hook 회귀 체크리스트: `docs/HOOK_TESTING.md`
- 협업 프로토콜 상세: `.claude/HYBRID.md`

## 버전 / 변경 정책

`v2.28 FROZEN` (Unified Claude + Codex Distribution). 변경 시 **새 버전 번호 + 변경 보고서 + Codex 리뷰** 절차를 따른다. 동결본을 직접 고치지 않는다.
