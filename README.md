...
# PawPad — Agentic Engineering Toolkit (v2.48 FROZEN)
...
실 프로젝트(Flutter 앱, Opus 부모 + Sonnet 서브에이전트, 다중 세션·407 테스트 규모)에서 기능별로 실측한 검증 리포트 — 전문: [`src/pawpad-token-analysis.md`](src/pawpad-token-analysis.md). ⬤=실측 ◐=추정.

| 기능 | 효과 | 근거 |
|------|------|------|
| **/code-delegate** (코딩 위임) | 동일 작업량 기준 부모 컨텍스트 **~30–40% 절감** + 슬라이스당 **71–93K 토큰을 하위 모델로 오프로드** | ⬤ 실측 4슬라이스 |
| **codemap** (심볼 레지스트리) | 사고(대형파일 통째읽기·오판·grep 난사) 봉쇄 시 **~48–88% 절감** — 정상 lookup은 중립 | ⬤ 실측 2케이스 |
| **ctxdb** (키워드 선택 로드) | 세션 시작 컨텍스트 **~89–96% 절감** (실측 로드 8.9KB vs 오리엔테이션 코퍼스 82–208KB) | ⬤ 로드분 실측 |
| **checkpoint/handoff + ON START** (세션 재개) | 콜드 재개 대비 재오리엔테이션 **~50–80% 절감** + 결정 손실 0 | ◐ 추정 |
| **caveman** (출력 압축) | 응답 출력 토큰 **~20–40%/턴** | ◐ 추정 |
| **종합** (중·대형 세션) | 부모 토큰 **~40–60% 절감** + 코딩 본체(71–93K/슬라이스) 격리 | 실측+추정 종합 |

- **정직 고지**: 절감은 대부분 **부모(상위 모델) 컨텍스트** 기준 — 위임 시 총 compute는 더 클 수 있다(효과의 본질 = 고가 모델 컨텍스트 보존 + 사용 한도 연장). codemap 절감은 **조건부**(기대절감 = 사고 확률 × 절감액 — 코드베이스 클수록↑, 작고 라벨 명확한 작업은 ~0). 단일 프로젝트·소표본 실측이다.
- **정직한 결론**: 작은 프로젝트·단발 세션엔 프로토콜 오버헤드가 더 클 수 있다. 이 툴킷의 체급은 **누적되는 프로젝트 + 다세션 + (선택) 멀티에이전트**. 토큰보다 먼저 얻는 것은 "끊겨도 정확히 이어짐"이다.
...
.\pawpad-setup.ps1 -Stack generic        # 프리셋: flutter | node | python | wpf | tauri | electron | avalonia | generic
...
pawpad-setup.ps1     # 통합 단일 설치 스크립트 (FROZEN v2.48, 번들 선택 -Preset/-Bundles + 안내 언어 -Lang, 설치 UI: 회전 paw 배너+live 진행 바+체크리스트, -ShowLog로 상세 로그)
.codex/
...
| 스킬 | 용도 |
|------|------|
| `/resume` | 세션 시작 재개 프로토콜 (HYBRID/_wip/lane/meta/codemap 읽기) + Lane 신뢰성 게이트(stale lane 감지·실코드 대조) |
| `/task-done` | **작업 종결 게이트** — ON TASK DONE 체크리스트 강제(lane→done 이관+_wip 제거+_meta+tasklog+codemap+commit). "작업/이슈 종료" 자연어 발동, Stop hook lane-close 백스톱과 짝 |
...
| `/design` | 화면/컴포넌트 UI/UX 설계·**시각 품질** 게이트 (방향→토큰→레이아웃→일관성 5축→anti-slop·정량 체크) — "AI 티=불일치" 차단 |
...
| 스킬 | 용도 |
|------|------|
| `/brainstorming` | 아이디어 발산 게이트 — 방향 발산(2-3 대안+추천 1개) + 누락 스윕(인접기능·비해피패스 8축) + MoSCoW 스코프 확정(Won't 명시). clarity 이전 단계, 구체화된 문서는 스윕 직행 |
...
```
아이디어
  └ /brainstorming  발산(방향 2-3 대안) + 누락 스윕(인접기능·비해피패스 8축) + MoSCoW 스코프 확정
                    막연한 아이디어는 발산부터, 구체화된 기획문서(.md)는 스윕 직행
...
구현
  ...
  └ /task-done      DoD 충족 → 종결 게이트 일괄 (lane wip/done/ 이동 + _meta 기록 + commit)
```
...
## 세션 운영 리듬 — task-done / checkpoint / handoff

하루 사용 패턴은 이 세 가지 구분이 전부다:

| 상황 | 명령 | 하는 일 |
|------|------|--------|
| 기능/이슈가 **끝났다** | `/task-done` | 종결 게이트 — lane→done 이관 + 이력 기록 + commit 일괄 |
| 작업 **도중** context 50~60% | `/checkpoint` → 새 세션 → `/resume` | lane/codemap/meta 저장 — **같은 agent**가 이어감 (snapshot 없음, owner 유지) |
| **다른 agent**(Claude↔Codex)로 넘김 | `/handoff codex {feature} [사유]` → 수신측 `/resume` | snapshot 작성 + **owner 이전** |

```
작업 완료      → /task-done → 새 세션 → /resume          (checkpoint 불필요 — 닫힌 lane엔 저장할 게 없음)
작업 도중 50%+ → /checkpoint → 새 세션 → /resume → 계속   (같은 agent)
agent 교체     → /handoff codex {feature} → (Codex) /resume 인수 (owner 변경 확인)
```

- **checkpoint vs handoff**: 같은 agent 새 세션 = checkpoint(저렴, 파일 갱신만). 다른 agent 또는 복잡한 미완 상태(검증 실패·Known Issues) = handoff(snapshot의 Completed/Remaining/Next Commands가 가치). 매번 handoff는 낭비.
- 기획만 끝난 상태는 둘 다 아님 — `/to-prd`가 lane을 `SPEC_READY`로 두면 구현 세션이 `/resume`으로 인수한다.
- context %는 statusline(`ctx 31% (62k/200k)`)으로 확인.
...
# 스크립트 STATUS
Get-Content .\pawpad-setup.ps1 -TotalCount 2   # v2.48 Unified ... FROZEN
```
...
- 변경 이력: `docs/CHANGELOG_v2.48.md` (이전: v2.18~v2.47)
- hook 회귀 체크리스트: `docs/HOOK_TESTING.md`
- 협업 프로토콜 상세: `.claude/HYBRID.md`

## 버전 / 변경 정책

`v2.48 FROZEN` (Unified Claude + Codex Distribution). 변경 시 **새 버전 번호 + 변경 보고서 + Codex 리뷰** 절차를 따른다. 동결본을 직접 고치지 않는다.
