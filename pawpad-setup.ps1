# PawPad — Agentic Engineering Toolkit | Setup Script v2.33 (Unified Claude + Codex Distribution, PowerShell)
# STATUS: FROZEN (v2.33. v2.32 기반 + code-delegate 스킬 신규(18→19) — 코딩 단계 진입 시 사용자 선택 모델의 코딩 서브에이전트로 위임(설계는 상위 Opus, 코딩은 하위 모델). 서브 독립 context로 부모 컨텍스트 린 유지 + 토큰 절감. spec/lane 포인터 전달·요약 반환·부모 피드백 루프. 접근법: instruction 스킬 + CLAUDE/AGENTS 자동제안(구현 진입 경계) 1줄. Claude Code 주력(Task model 오버라이드)·Codex 수동 세션 폴백. 배포 표면: SKILL live+임베드+.agents 미러, 자동제안 4표면, SKILLS_MANIFEST·config skills 19, README/GUIDE/USAGE, codemap. 보고서: docs/CHANGELOG_v2.33.md).
...
# - .claude/skills/* (memory, codemap, codebase-map, caveman, lean-code, feature-architecture, clarity, handoff, checkpoint, grill-me, grill-with-docs, to-prd, design, mockup, review, code-delegate, ctxdb-navigator, context-saver, security-check)
...
- 리뷰측은 보통 별도 호출 없이 memory ON START가 REVIEW_REQUESTED를 발견해 진입.
