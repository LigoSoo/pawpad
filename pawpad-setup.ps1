# PawPad — Agentic Engineering Toolkit | Setup Script v2.23 (Unified Claude + Codex Distribution, PowerShell)
# STATUS: FROZEN (v2.23. v2.22 기반 + 설치 UI: paw 배너(truecolor/ConsoleColor 폴백) + 28단계 실시간 진행 바 + 실측 결과 체크리스트(✓/-/✗). 설치 내용물 변경 없음. 보고서: docs/CHANGELOG_v2.23.md).
#         이전: v2.22 구조 코드명 KMS 완전 제거(.claude/KMS->.claude/pawpad, kms-config->pawpad-config, codemap 접두/스킬 scope kms->pawpad, -Upgrade 자동 마이그레이션. ADR-002. 보고서: docs/CHANGELOG_v2.22.md).
...
# Usage: .\pawpad-setup.ps1 [-Stack <flutter|node|python|generic>] [-Force | -Upgrade]
...
# - .claude/skills/* (memory, codemap, caveman, karpathy, clarity, handoff, checkpoint, grill-me, grill-with-docs, to-prd, design, ctxdb-navigator, context-saver, security-check)
...
param([switch]$Force, [switch]$Upgrade, [string]$Stack = "")
...
function Step-Begin {
    ...
    $bar = ('█' * $fill) + ('░' * ($width - $fill))
    Write-Host ""
    Write-UiLine ("  [{0}] {1,3}%  ({2,2}/{3})  {4}" -f $bar, $pct, $idx, $script:stepTotal, $Label) $script:uiMint Cyan
}
...
function Show-InstallChecklist {
    ...
    Complete-CurrentStep
    Write-Host ""
    Write-UiLine ("  [{0}] 100%  ({1}/{1})  complete" -f ('█' * 30), $script:stepTotal) $script:uiMint Cyan
    Write-Host ""
...
function Write-FileContent {
    ...
    if ($writeMode -eq 'MERGE-PENDING') {
        ...
        Write-Host "  MERGE-Q $Path (혼합 파일, 설치 후 병합)" -ForegroundColor DarkCyan
        ...
        try {
            ...
            # 쓰기 실패를 성공으로 보고하지 않음 (sandbox/권한 환경에서 Access denied 가능)
...
function Merge-MdToolkitSections {
    ...
    Write-Host "  MERGED  $Path (toolkit sections: $($ToolkitSections.Count))" -ForegroundColor Cyan
    $script:merged++
}
...
function Merge-JsonToolkitKeys {
    ...
    try {
        ...
        Write-Host "  MERGE-FAIL $Path (JSON parse 실패 - 수동 병합 필요. 백업 참조)" -ForegroundColor Red
        $script:failed++
        return
    }
    ...
    Write-Host "  MERGED  $Path (toolkit keys: $($ToolkitKeys -join ','))" -ForegroundColor Cyan
    $script:merged++
}
...
function Update-Gitignore {
    ...
    if (Test-Path $gitignorePath) {
        ...
        if ($newEntries.Count -gt 0) {
            ...
            Add-Content -Path $gitignorePath -Value $append -Encoding UTF8 -NoNewline
            Write-Host "  UPDATED .gitignore (PawPad entries appended)" -ForegroundColor Yellow
            $script:created++
        } else {
            Write-Host "  SKIP    .gitignore (PawPad entries already present)" -ForegroundColor DarkGray
            $script:skipped++
        }
    } else {
        $content = @"
...
# ── Skills: karpathy ──────────────────────────────────────────────────────────
Step-Begin "skill: karpathy"
Write-FileContent ".claude\skills\karpathy\SKILL.md" -NoBom @"
---
name: karpathy
...
- karpathy 적용: 요청된 화면만, 과한 애니메이션/추상화 금지.
...
if (Test-Path $sourceSkillRoot) {
    ...
    Write-Host "  CODEX SKILLS mirror source=$($skillDirs.Count) mirror=$mirrorCount" -ForegroundColor Cyan
    if ($mirrorCount -lt $skillDirs.Count) {
        Write-Host "  WARNING mirror 불완전 ($mirrorCount/$($skillDirs.Count)) - .agents/skills 쓰기 실패 여부 위 FAILED 라벨 확인" -ForegroundColor Yellow
    }
}
...
if ($Upgrade -and $mergePending.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "Merging mixed files (toolkit sections/keys only)..." -ForegroundColor White
    $claudeToolkitSections = @(
        'Definition of Done', 'Escalation Rules', 'Coding Principles (Karpathy)',
...
if ($failed -eq 0) {
    ...
    Write-Host "  - 설치 UI: paw 배너 + 28단계 실시간 진행 바 + 실측 결과 체크리스트" -ForegroundColor Cyan
    Write-Host "  - 상세: docs/CHANGELOG_v2.23.md" -ForegroundColor Cyan
    Write-Host ""
} else {
