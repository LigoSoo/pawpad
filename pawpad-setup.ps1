# PawPad — Agentic Engineering Toolkit | Setup Script v2.28 (Unified Claude + Codex Distribution, PowerShell)
# STATUS: FROZEN (v2.28. v2.27 기반 + 신규 mockup 스킬(PRD-tree→단일 HTML 목업, lo/hi-fi 인자+hi 자동제안, Feature ID 태깅 + 단방향 drift 경고, src/mockups/{feature-id}-mockup.html) + CLAUDE/AGENTS Idea → PRD Routing 확장(design/mockup 추천 + ### 자동제안[단계경계: 문서갱신/스킬종료 1회, 거절 시 다음경계 침묵, 기획·설계 흐름 스킬 한정] + ### 선택지 질문=체크박스 + Active Skills 첨자 design·mockup). 자동제안=instruction 기반(hook 범위 외). 배포 표면 동기: 임베디드 $tmplClaudeMd/$tmplAgentsMd, mockup 스킬 임베드(@'...'@ literal), SKILLS_MANIFEST 16→17, .codex/config.json skills, .agents 미러, codemap, README/GUIDE/USAGE. Codex review-01 PASS_WITH_FIXES 92%(코드 결함 0). 보고서: docs/CHANGELOG_v2.28.md).
#         이전: v2.27 CLAUDE/AGENTS Idea → PRD Routing + Active Skills 표시 추가 + doc/스킬 군살 제거. 보고서: docs/CHANGELOG_v2.27.md).
...
# - .claude/skills/* (memory, codemap, codebase-map, caveman, lean-code, feature-architecture, clarity, handoff, checkpoint, grill-me, grill-with-docs, to-prd, design, mockup, ctxdb-navigator, context-saver, security-check)
...
if ($failed -eq 0) {
    ...
    Write-Host ""
    Write-Host "v$ver 누적 (17 스킬 + hook + .ctxdb + codemap + codebase-map + security-check):" -ForegroundColor Cyan
    ...
    Write-Host "  - 상세: docs/CHANGELOG_v2.28.md" -ForegroundColor Cyan
    Write-Host ""
} else {
