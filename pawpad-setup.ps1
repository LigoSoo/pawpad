...
#         - Stack 프리셋: flutter | node | python | wpf | tauri | electron | avalonia | generic (생략 시 대화형 선택)
...
$script:uiAmber = '38;2;252;175;3'     # docs/icon.png 브랜드 마크 앰버
$script:uiMint  = '38;2;126;200;177'   # 구분선/진행 바 민트

# 회전 프레임 데이터: docs/icon.png(브랜드 마크) -> 47×24 chars ASCII, 16장.
# 생성 방식: 셀 단위 alpha coverage 샘플링(luma cut 0.35 -> 패드의 >_ 프롬프트를 구멍으로 보존),
#           소스를 가로로 cos(theta) 압축해 각도별로 재변환(2D 코인 스핀, cos<0은 좌우 반전).
# 전 프레임 줄 수는 24로 동일해야 한다 - 커서 되감기 ESC[24A 가 줄 수에 의존한다.
$script:pawFrames = @(
    #   0 deg
    , @(
        '              .;**~,',
...
function Show-PawBanner {
    # 프레임 스와핑 회전: 프레임을 그린 뒤 ESC[{h}A 로 커서를 되감아 같은 자리에 다음 프레임을 덮어쓴다.
    # Step-Begin 의 한 줄 제자리 갱신(CR)을 세로로 확장한 것 - 3D 렌더가 아니라 2D 코인 스핀이다.
    $rule = '  ' + ('═' * 44)
    $frames = $script:pawFrames
    $h = $frames[0].Count
    ...
    # 창 높이가 프레임보다 낮으면 되감기가 스크롤과 엉켜 화면이 깨진다 -> 정지 프레임으로 폴백
    $tall = $false
    try { $tall = [Console]::WindowHeight -ge ($h + 12) } catch {}

    if ($script:uiLive -and $tall) {
        $seq = @(0..($frames.Count - 1)) + 0   # 1회전 후 0도 정면에 정착
        for ($i = 0; $i -lt $seq.Count; $i++) {
            foreach ($line in $frames[$seq[$i]]) {
                # ESC[K: 좁은 프레임이 넓은 프레임 위에 덮일 때 남는 잔상 제거
                Write-Host ("{0}[{1}m{2}{0}[0m{0}[K" -f $script:E, $script:uiAmber, $line)
            }
            if ($i -lt $seq.Count - 1) {
                Start-Sleep -Milliseconds 180
                Write-Host -NoNewline ("{0}[{1}A" -f $script:E, $h)
            }
        }
    } else {
        foreach ($line in $frames[0]) { Write-UiLine $line $script:uiAmber Yellow }
    }
...
# ── Stack 선택 (v2.15) ─────────────────────────────────────────────────────────
$validStacks = @('flutter', 'node', 'python', 'wpf', 'tauri', 'electron', 'avalonia', 'generic')
if (-not $Stack) {
    Write-Host ""
    ...
    switch -Regex ($sel.Trim().ToLower()) {
        ...
        '^(4|wpf)$'      { $Stack = 'wpf' }
        '^(5|tauri)$'    { $Stack = 'tauri' }
        '^(6|electron)$' { $Stack = 'electron' }
        '^(7|avalonia)$' { $Stack = 'avalonia' }
...
$profiles['wpf'] = @{
    Label = 'WPF (.NET 8) + MVVM'
    Commands = @'
- Install:     dotnet restore
- Dev:         dotnet run --project src/App
- Test:        dotnet test
- Test single: dotnet test --filter "FullyQualifiedName~TestName"
- Analyze:     dotnet build -warnaserror
...
- bin/ , obj/
- *.g.cs (XAML generated)
- packages.lock.json (without instruction)
'@
    NeverRun = @'
- Production publish / installer upload
- Code signing scripts
- Destructive data scripts
'@
    Directories = @'
src/
├── App/          # entry, App.xaml, DI bootstrap
├── Features/     # feature-first: View + ViewModel + Service colocation
├── Core/         # shared services, base classes
└── Resources/    # styles, themes (ResourceDictionary)
'@
    Conventions = @'
- MVVM 고정: code-behind 로직 금지, CommunityToolkit.Mvvm([ObservableProperty]/[RelayCommand])
- View/ViewModel 페어: FooView.xaml + FooViewModel.cs, PascalCase
- nullable enable, no dynamic
- 스타일은 ResourceDictionary 집중 - 인라인 스타일 금지
'@
    AnalyzePS   = 'dotnet build -warnaserror | Select-Object -Last 5'
    AnalyzeBash = 'dotnet build -warnaserror | tail -5'
    MetaStack   = 'WPF+.NET8+MVVM'
    StackInfo = @'
    "framework": "WPF (.NET 8)",
    "language": "C#",
    "stateManagement": "MVVM (CommunityToolkit.Mvvm)",
    "database": "-"
'@
    VerifyCmds = '["dotnet build -warnaserror", "dotnet test"]'
    Adr = @'
## ADR-001: UI 패턴 - MVVM (CommunityToolkit.Mvvm)
결정: View/ViewModel 분리, code-behind 로직 금지.
이유: 테스트 가능성, XAML 바인딩 일관성.
날짜: __TODAY__

## ADR-002: DI - Microsoft.Extensions.DependencyInjection
결정: Generic Host 기반 DI 컨테이너 사용.
이유: 서비스 수명 관리 일관, 테스트 대체 용이.
날짜: __TODAY__
'@
}
$profiles['tauri'] = @{
    Label = 'Tauri 2 (Rust + Web Frontend)'
    Commands = @'
- Install:     npm install
- Dev:         npm run tauri dev
- Test:        npm test  (Rust: cargo test --manifest-path src-tauri/Cargo.toml)
- Test single: npm test -- path/to/test.spec.ts
- Analyze:     npx tsc --noEmit  (Rust: cargo check --manifest-path src-tauri/Cargo.toml)
- Lint:        npx eslint .  (Rust: cargo clippy)
...
- package-lock.json , Cargo.lock (without instruction)
'@
    NeverRun = @'
- Production build & release (store / updater)
- Code signing scripts
- Destructive data scripts
'@
    Directories = @'
src/              # web frontend (feature-first)
├── features/
└── lib/
src-tauri/
├── src/          # Rust commands (IPC)
├── capabilities/ # permission scopes
└── tauri.conf.json
'@
    Conventions = @'
- IPC 경계 집중: #[tauri::command]는 src-tauri/src/commands/ 에만, 프론트 invoke 래퍼 1곳
- capabilities 최소 권한(필요 API만 등록)
- TypeScript strict / Rust clippy 경고 0
- 파일: 프론트 kebab-case.ts, Rust snake_case.rs
'@
    AnalyzePS   = 'npx tsc --noEmit | Select-Object -Last 5'
    AnalyzeBash = 'npx tsc --noEmit | tail -5'
    MetaStack   = 'Tauri2+Rust+TS'
    StackInfo = @'
    "framework": "Tauri 2",
...
'@
    VerifyCmds = '["npx tsc --noEmit", "npm test"]'
    Adr = @'
## ADR-001: 데스크탑 셸 - Tauri 2
결정: Electron 대신 Tauri 2.
이유: 시스템 webview 사용으로 메모리·설치본 크기 우위.
날짜: __TODAY__

## ADR-002: IPC 경계 - tauri::command 집중
결정: 커맨드는 commands/ 한 곳, capabilities 최소 권한.
이유: 권한 표면 최소화, 감사 용이.
날짜: __TODAY__
'@
}
...
- Analyze:     npx tsc --noEmit
- Lint:        npx eslint .
- Build:       npm run build  (packaging: npx electron-builder)
- Codegen:     (n/a)
'@
    NeverModify = @'
- node_modules/
...
'@
    NeverRun = @'
- electron-builder publish (auto-update release)
- Code signing scripts
- Destructive data scripts
'@
    Directories = @'
src/
├── main/        # main process (창/수명주기/IPC 핸들러)
├── preload/     # contextBridge API 노출
├── renderer/    # UI (feature-first)
└── shared/      # 타입/상수 공유
'@
    Conventions = @'
- 보안 기본값 고정: contextIsolation true, nodeIntegration false
- IPC는 preload contextBridge 화이트리스트 경유만 - 렌더러 Node API 직접 접근 금지
- 원격 콘텐츠 로드 금지(로컬 번들만)
- TypeScript strict, 파일 kebab-case.ts
'@
    AnalyzePS   = 'npx tsc --noEmit | Select-Object -Last 5'
    AnalyzeBash = 'npx tsc --noEmit | tail -5'
    MetaStack   = 'Electron+TypeScript'
    StackInfo = @'
    "framework": "Electron",
    "language": "TypeScript",
    "stateManagement": "-",
...
'@
    VerifyCmds = '["npx tsc --noEmit", "npm test"]'
    Adr = @'
## ADR-001: 프로세스 경계 - main/preload/renderer 3분할
결정: contextIsolation true + preload 화이트리스트 IPC.
이유: 렌더러 침해 시 Node API 접근 차단(보안 격리).
날짜: __TODAY__

## ADR-002: 패키징 - electron-builder
결정: electron-builder로 인스톨러/업데이트 생성.
이유: 크로스플랫폼 인스톨러·자동업데이트 사실상 표준.
날짜: __TODAY__
'@
}
$profiles['avalonia'] = @{
    Label = 'Avalonia UI (.NET 8) + MVVM'
    Commands = @'
- Install:     dotnet restore
- Dev:         dotnet run --project src/App
- Test:        dotnet test
...
'@
    Directories = @'
src/
├── App/          # entry, App.axaml, DI bootstrap
├── Features/     # feature-first: View(.axaml) + ViewModel colocation
├── Core/         # shared services, platform interfaces
└── Assets/       # styles, themes
'@
    Conventions = @'
- MVVM 라이브러리 단일화: CommunityToolkit.Mvvm 단독(ReactiveUI 혼용 금지)
- View/ViewModel 페어: FooView.axaml + FooViewModel.cs, PascalCase
- nullable enable
- 플랫폼 분기 최소화 - 플랫폼별 코드는 Core 인터페이스 뒤로
'@
    AnalyzePS   = 'dotnet build -warnaserror | Select-Object -Last 5'
    AnalyzeBash = 'dotnet build -warnaserror | tail -5'
    MetaStack   = 'Avalonia+.NET8+MVVM'
    StackInfo = @'
...
'@
    VerifyCmds = '["dotnet build -warnaserror", "dotnet test"]'
    Adr = @'
## ADR-001: 크로스플랫폼 UI - Avalonia
결정: WPF 유사 XAML로 Win/Mac/Linux 단일 코드베이스.
이유: WPF 경험 이식 + 크로스플랫폼 요구 충족.
날짜: __TODAY__

## ADR-002: MVVM 라이브러리 단일화
결정: CommunityToolkit.Mvvm 단독 사용.
이유: ReactiveUI 혼용 시 패턴 파편화.
날짜: __TODAY__
'@
}
$profiles['generic'] = @{
...
-> Detail: .claude/skills/resume/SKILL.md | .claude/skills/task-done/SKILL.md | .claude/skills/codemap/SKILL.md | .claude/skills/ctxdb-navigator/SKILL.md | .claude/skills/context-saver/SKILL.md | .claude/skills/handoff/SKILL.md | .claude/skills/checkpoint/SKILL.md
...
2. UI/화면 포함 시 design 1회 추천.
3. 코딩 진입 시 code-delegate 1회 권장(외부 참조 문서 = written 설계로 인정).
phase 분해·task 저장만으로 이 게이트를 건너뛰지 않는다.
### 자동제안 (단계 경계)
...
### 선택지 질문 = 체크박스
스킬 진행 여부와 무관하게 **사용자 결정이 필요한 선택지 N개 질문은 AskUserQuestion(체크박스)** 로 받는다. 추천 1개를 첫 옵션 배치 + 라벨 끝 "(추천)" 표기 + description에 근거. 선택지 밖 답은 도구 기본 제공 "Other"로 자유 입력 가능 — 선택지 생략이나 산문 질문으로 대체 금지. 자유서술·수치 입력은 텍스트 유지.
...
# task-done: resume 단계 동거 설치 (v2.43 신규, ON TASK DONE 실행 게이트 — stepTotal 불변, v2.36 viewer-apply 선례)
Write-FileContent ".claude\skills\task-done\SKILL.md" -NoBom @'
---
name: task-done
...
- 자동 발동(외부 문서 모드): 외부 문서 참조 구현 요청 시 명시 호출 없이 채점 의무. 아래 "외부 문서 모드" 참조.

## 외부 문서 모드 (자동 발동, 채점 의무)
사용자가 외부 문서(첨부 md/spec/기획서 경로)를 참조해 구현을 요청하면 코딩 시작 전 이 모드로 1회 채점한다 (CLAUDE.md/AGENTS.md "외부 문서 구현 진입 게이트").
- 채점 대상 = 문서 내용 + 대화 보충 정보. 문서에 명시된 정보는 명확으로 인정 (채점 원칙의 "요청 텍스트" = 문서 포함).
- 모호도 블록 출력은 의무, 재질문은 조건부: PASS(≤임계값)면 질문 없이 접근법 판정/구현 진행 (마찰 최소), BLOCK이면 일반 흐름과 동일하게 재질문 라운드.
- phase 분해·task 저장을 했더라도 이 채점을 건너뛰지 않는다.
- 문서가 UI/화면을 포함하거나 코딩에 진입할 때의 후속 스킬 추천은 CLAUDE.md 자동제안 규칙을 따른다(설치된 스킬만 추천).
...
# -- Skills: design references (원형 카드·토큰 시작안·체크리스트, on-demand read) --
Write-FileContent ".claude\skills\design\references\archetypes.md" -NoBom @'
...
Write-FileContent ".claude\skills\design\references\tokens.md" -NoBom @'
...
Write-FileContent ".claude\skills\design\references\checklists.md" -NoBom @'
...
if ($script:bundleMode -eq 'custom') {
    ...
    foreach ($doc in @('CLAUDE.md', 'AGENTS.md', '.claude\HYBRID.md')) {
        ...
        # 외부 문서 구현 진입 게이트(v2.44) clause 정리: 미설치 스킬 이름 잔존 0
        if ($bpRemove -contains 'design') {
            $t = [regex]::Replace($t, '(?m)^2\. UI/[^\r\n]*design[^\r\n]*\r?\n', '')
            $t = [regex]::Replace($t, ' ?② UI/[^①②③\r\n]*design[^①②③\r\n]*(?=③|\. phase)', ' ')
            $t = $t -replace '\) \. phase', '). phase'
        }
        if ($bpRemove -contains 'code-delegate') {
            $t = [regex]::Replace($t, '(?m)^3\. 코딩 진입 시 code-delegate[^\r\n]*\r?\n', '')
            $t = [regex]::Replace($t, ' ?③ 코딩 진입 시 code-delegate[^①②③\r\n]*(?=\. phase)', '')
        }
        # 자동제안 리뷰/코딩 위임 clause 정리(선재 잠복: `/review`·`/code-delegate`는 backtick-strip `skill 패턴 미매칭 — prd-only 조합서 노출)
        if ($bpRemove -contains 'review') {
            $t = [regex]::Replace($t, '(?m)^- 리뷰 제안\(구현완료 경계\):[^\r\n]*\r?\n', '')
            $t = [regex]::Replace($t, '리뷰 제안\(구현완료 경계\):[^\r\n]*?에스컬레이션\. ?', '')
        }
        if ($bpRemove -contains 'code-delegate') {
            $t = [regex]::Replace($t, '(?m)^- 코딩 위임 제안\(구현 진입 경계\):[^\r\n]*\r?\n', '')
            $t = [regex]::Replace($t, ' ?코딩 위임 제안\(구현 진입 경계\):[^\r\n]*?제안 X\)\.', '')
        }
...
if ($failed -eq 0) {
    ...
    if ($Lang -eq 'ko') {
        ...
        Write-Host "  - brainstorming skill (v2.45): idea divergence + omission sweep (adjacent features, 8-axis non-happy-path checklist) + MoSCoW scope gate in one skill — pre-clarity stage, cuts late feature add/remove churn (prd bundle, 20->21)" -ForegroundColor Cyan
        Write-Host "  - design skill visual-quality redesign (v2.46): top-3 principles (consistency-first / intentional direction / anti-slop) + 5-axis consistency scales (token-derived values only, no arbitrary values) + quantitative checks (radius<=4, control heights<=3, zero arbitrary spacing) + 2-pass workflow (skills stay 21) | details: docs/CHANGELOG_v2.46.md" -ForegroundColor Cyan
    }
    Write-Host ""
} else {
