...
if ($failed -eq 0) {
    ...
    if ($Lang -eq 'ko') {
        ...
        Write-Host "  - 상세: docs/CHANGELOG_v2.43.md" -ForegroundColor Cyan
    } else {
        ...
        Write-Host "  - task-done closure gates (v2.43): new task-done skill (enforced ON TASK DONE checklist) + Stop-hook lane-close backstop (completion declared + lane remaining -> one reminder) + resume lane-trust gate (stale lane detection vs real code) | details: docs/CHANGELOG_v2.43.md" -ForegroundColor Cyan
    }
    Write-Host ""
} else {
